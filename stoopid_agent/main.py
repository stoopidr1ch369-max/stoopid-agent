...
money = [
    "💰 STOOPID R1CH money talk active.",
...
def handle(text):
    t = text.lower()
    if "💰" in text:
        return pick("money", money)
    if "$" in text:
        return pick("dollar", dollar)
    ...
    if "news" in t or "update" in t or "status" in t:
        return pick("news", news)
    if "trivia" in t:
        return pick("trivia", trivia)
    if "task" in t:
        return pick("tasks", tasks)
    return None
...
async def ca_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(pick("ca_cmd", ca))
...
async def news_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(pick("news_cmd", news))

async def trivia_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(pick("trivia_cmd", trivia))

async def tasks_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(pick("tasks_cmd", tasks))
